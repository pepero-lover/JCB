...
- On `MoveInfo`, added `isWhiteMoving` to check who played this move.
- On `ChessGame`, added editing pgn data methods with node id (`setMoveClockAt`, `setMoveEvalAt`, ...)
...
- On `SyzygyAnalyzer`, changed just one repetition checking to three-fold repetition checking.
- And also `GaviotaAnalyzer`, changed checking three-fold repetition.
...
- On `ChessGame`, fixed checking variant outside of `readLock`.
